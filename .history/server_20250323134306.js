...
function onConnected(socket) {
  ...
  checkAuthenticated(socket.request, null, () => {
    const user = socket.request.user;
    const verifiedUser = {
      username: user.name,
      socketId: socket.id,
    }
    userSockets.add(verifiedUser)
    ...
    socket.on('disconnect', () => {
      ...
      userSockets.delete(verifiedUser)
      io.emit("total-clients", socketsConnected.size)
    })

    socket.on('message', (roomName, data) => {
      console.log(data)
      socket.to(roomName).emit('chat-message', data)
    })

    socket.on('feedback', (roomName, data) => {
      socket.to(roomName).emit('feedback', data)
    })
  });
}
