...
function onConnected(socket) {
  ...
  socket.on('join-room', (room) => {
    console.log(`User ${socket.id} is attempting to join room ${room}`);
    if (rooms[room]) {
      socket.join(room);
...
// Rooms
app.get('/rooms/:room', checkAuthenticated, (req, res) => {
  res.render('index', {name: req.user.name, roomName: req.params.room, roomMessages: rooms[req.params.room].messages})
})

// Authentication

app.get('/', checkAuthenticated, (req, res) => {
  res.render('room.ejs', { name : req.user.name, rooms: rooms})
})
